...
const REPLIT_URL = process.env.REPLIT_URL || `https://${process.env.REPL_SLUG}-${process.env.REPL_OWNER}.${process.env.REPL_SLUG}.repl.co`;
...
app.post('/api/chat-ai', async (req, res) => {
  ...
  try {
    ...
    // Llamar al API de IA
    const response = await fetch(`${aiModel.endpoint}?query=${encodeURIComponent(message)}`);
