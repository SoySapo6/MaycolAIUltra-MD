...
if (args.length < 1) return conn.reply(m.chat, '💣 Ingresa la cantidad de ' + `🍪 *Cookies*` + ' que deseas aportar contra' + ` *MaycolAIUltra-MD*` + `\n\n` + '`Ejemplo:`\n' + `> *${usedPrefix + command}* 100`, m, rcanal)
if (user.cookies >= count * 1) {
user.cookies -= count * 1
if (Aku > Kamu) {
conn.reply(m.chat, '`💣 Veamos que numeros tienen!`\n\n'+ `➠ *MaycolBot* : ${Aku}\n➠ *${username}* : ${Kamu}\n\n> ${username}, *PERDISTE* ${formatNumber(count)} 🍪 Cookies.`.trim(), m, rcanal)
} else if (Aku < Kamu) {
